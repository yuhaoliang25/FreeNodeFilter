...
默认使用 `https://www.google.com/generate_204`。每个节点先进行一次全量 Stage 1 测试；成功节点进入 Stage 2，最多保留 300 个；其中最多 100 个进入第 3 轮深测，因此高质量节点本轮最多有 3 次实际测试。Mihomo 的 group delay API 可以一次测试指定代理组中的节点，并支持 `expected` 状态码；这里要求 HTTP 204。citeturn0search0
...
本轮结构：`全部候选 → Stage 1 → 成功节点 → Stage 2（最多 300） → Stage 3（最多 100）`。`stable.yaml` 要求当前批次至少完成 2 次测试，并同时满足当前成功率、历史成功率和延迟条件；`best.yaml` 要求当前批次完成 3 次测试，并使用更严格的成功率和延迟条件。
