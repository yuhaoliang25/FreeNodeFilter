...
`data/reputation.json` 会根据历史测试维护节点状态。总测试次数仍保留用于审计，但质量判断主要使用最近 12 次实际测试的递增权重，越新的测试权重越高。最近 6 次全部失败会进入 `quarantine`；最近 6 次至少 3 次失败，或加权成功率低于 60%，进入 `degraded`；最近 6 次中出现失败但仍保持较高恢复能力的节点可标记为 `flaky`。被隔离的节点不会进入 `best.yaml`。
