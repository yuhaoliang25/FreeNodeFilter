...
本轮结构：`全部候选 → Stage 1 → Stage 1 失败重试 → Stage 2（最多 300） → Stage 3（最多 100）`。单次瞬时超时不会直接淘汰节点。`stable.yaml` 要求当前批次至少完成 2 次测试，并同时满足当前成功率、历史成功率和延迟条件；`best.yaml` 要求当前批次完成 3 次测试，并使用更严格的成功率和延迟条件。
