...
`sources/sources.yaml` 只作为初始种子来源。GitHub Actions 每次运行前会执行来源发现：从公开 GitHub 仓库中寻找可能包含订阅/节点列表的文件，下载并检查其是否确实包含可解析的节点内容，再加入 `data/sources.json` 动态来源池。动态来源使用 URL 哈希作为稳定身份，避免文件显示名变化造成重复。
