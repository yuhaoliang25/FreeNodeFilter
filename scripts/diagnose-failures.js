...
function classify(stderr,code,httpCode){
 ...
 if(httpCode&&httpCode!=='204')return 'http_'+httpCode;
 ...
 for(const n of failed){
  try{
   ...
   const p=await new Promise(resolve=>{
    ...
    let err='',out='';cp.stdout.on('data',d=>out+=d);cp.stderr.on('data',d=>err+=d);cp.on('close',code=>{const m=out.trim().match(/^(\d{3})\s+([0-9.]+)$/);resolve({code,stdout:out,httpCode:m?.[1]||null,timeTotal:m?.[2]?Number(m[2]):null,stderr:err})});
   });
   const ok=p.code===0&&p.httpCode==='204';
   report.results[n.fingerprint||n.name]={name:n.name,diagnostic:ok?'http_success':classify(p.stderr,p.code,p.httpCode),exitCode:p.code,httpCode:p.httpCode,timeTotal:p.timeTotal,stderr:p.stderr.slice(0,300)};
